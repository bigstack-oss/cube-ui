...
- **Docs page**: every `meta.parameters.docs.page` renders the same four blocks in order — `<Title /> <Description /> <Primary /> <Controls />` — from `@storybook/addon-docs/blocks`.
...
- **Two stories per component**: `Playground` and `Gallery`.
  ...
  - `Gallery` — exhaustive static visual reference, `tags: ['!autodocs']`, controls/actions/interactions all disabled via `parameters`, built with `<StoryLayout>` / `<StoryLayout.Section>`.
